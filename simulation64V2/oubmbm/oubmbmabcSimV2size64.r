...
setwd("~/Dropbox/FCU/Teaching/Mentoring/2019Spring/ChihPingWang/Model1/abc_V2/simulation64V2/oubmbm/")
...
oubmbmprior<-function(prior.model.params=prior.model.params,prior.reg.params=prior.reg.params){
  ...
  sigma.sq.x<-rinvgamma(n=1, shape=sigmasq.x.shape, scale=sigmasq.x.scale)
  ...
  model.params<-c(alpha.y, sigma.sq.x, tau)
...
prior.model.params=c(alpha.y.rate, sigmasq.x.shape, sigmasq.x.scale, tau.shape, tau.scale)
names(prior.model.params)<-c("alpha.y.rate", "sigmasq.x.shape", "sigmasq.x.scale", "tau.shape", "tau.scale")
...
true.trait<-oubmbmmodel(model.params=c(true.alpha.y,true.sigmasq.x,true.tau),reg.params=c(true.b0,true.b1,true.b2),root=root,tree=tree)
...
rownames(model.params.array)<-c("alpha.y","sigmasq.x","tau")
...
rej <- abc(target=c(raw.sum.stat.y,raw.sum.stat.x1,raw.sum.stat.x2), param=oubmbm.par.sim, sumstat=sim.sum.stat, tol=tol, method="rejection")
post.oubmbm <- as.data.frame(rej$unadj.values)

save.image(paste("oubmbmsimV2size",n,".RData",sep=""))

# ### Bayesian Credible Interval for alpha.y
# postTheta <- data.frame(rej$unadj.values)
# rep<-50
# post_alpha.y <- sample(postTheta$alpha.y,rep,replace=FALSE)
# sample_alpha.y <- matrix(NA,rep,sims*tolrate)
# dim(sample_alpha.y)
#
...
#   #repIndex<-1
#
#   true.alpha.y<- post_alpha.y[repIndex]
#   true.trait<-oubmbmmodel(model.params=c(true.alpha.y,true.sigma.sq.x,true.tau),reg.params=c(true.b0,true.b1,true.b2),root=root,tree=tree)
#
#   raw.sum.stat.y<-sum.stat(trait=true.trait$y,tree=tree)
#   raw.sum.stat.x1<-sum.stat(trait=true.trait$x1,tree=tree)
#   raw.sum.stat.x2<-sum.stat(trait=true.trait$x2,tree=tree)
...
#
#     sim.alpha.y<-rexp(n=1,rate=alpha.y.rate)
#
#     model.params<-c(sim.alpha.y,true.sigma.sq.x,true.tau)
...
#     sim.trait <-oubmbmmodel(model.params=model.params,reg.params=reg.params,root=root,tree=tree)
#     sim.oubmbm.trait[,1,simIndex]<-sim.trait$y
#     sim.oubmbm.trait[,2,simIndex]<-sim.trait$x1
#     sim.oubmbm.trait[,3,simIndex]<-sim.trait$x2
#     y.sum.stat.array[,simIndex]<- sum.stat(trait=sim.trait$y,tree=tree)
...
#   sim.sum.stat <- cbind(t(y.sum.stat.array),t(x1.sum.stat.array),t(x2.sum.stat.array))
#   oubmbm.par.sim <- cbind(t(model.params.array),t(reg.params.array))
#   rej <- abc(target=c(raw.sum.stat.y,raw.sum.stat.x1,raw.sum.stat.x2), param=oubmbm.par.sim, sumstat=sim.sum.stat, tol=tolrate, method="rejection"  )
#   postTheta_sim <- data.frame(rej$unadj.values)
#   sample_alpha.y[repIndex,] <- postTheta_sim$alpha.y
# }
