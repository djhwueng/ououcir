...
# Main
reps<-1000
treesize.array<-c(5,10,30,50,100)
...
for(treesizeIndex in 1:length(treesize.array) ){
  ...
  # Cross-Validation
  cv.mod <- cv4postpr(modelset,stat,nval=100,tols=0.01,method="mnlogistic")
  s<-summary(cv.mod)
...
cv.mod$model.probs
cv.mod$tols
plot(cv.mod,names.arg=c("oubmbm","oubmcir","ououbm","ououcir"))
...
for(sizeIndex in 1:length(size)){
  ...
  barplot(t(sdata),col=c("pink","skyblue","orange","green"),main=paste("taxa size ", size[sizeIndex]), xlab="", ylab="",beside=TRUE)
